...
export const getDashboardStats = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const [
      ...
      prisma.payout.aggregate({ _sum: { amount: true }, where: { status: { in: ['PENDING'] } } }),
      prisma.payout.count({ where: { status: { in: ['PENDING', 'PAID'] } } }),
    ...
    sendSuccess(res, {
      metrics: {
        ...
        candidateGeography: locationRows.map((r) => ({ location: r.location || 'Not specified', count: r._count?._all ?? Object.values(r._count ?? {}).reduce((sum, value) => sum + Number(value || 0), 0) })),
